...
- Each watch/category has its own "how far above floor is still OK"
  ...
  for.
- The floor used for those thresholds is computed from UNRESTRICTED
  on-sale listings only (see compute_unrestricted_floors). Marketapp's own
  precomputed floor is only used as a fallback.
"""
...
def check_watches(notified_prices: dict[str, tuple]) -> None:
    ...
    # Fetch the GRAM->USD rate and Marketapp's own floor price, but only for
    # the collection(s) we're actually watching (e.g. just Anonymous
    # Numbers), not every collection on the site. The API floor is only a
    # FALLBACK now -- the floor actually used is computed below from
    # unrestricted listings. `or {}` also prevents an AttributeError when
    # the request fails and get_collection_floors() returns None.
    gram_usd_rate = get_gram_usd_rate()
    api_floors = get_collection_floors(watched_collections, debug=DEBUG) or {}
    if DEBUG:
    ...
    # Floor from unrestricted listings only; fall back to the API's floor
    # for any collection where we couldn't compute one.
    collection_floors = {**api_floors, **compute_unrestricted_floors(collection_items, gram_usd_rate)}
    if DEBUG:
        for address, floor in collection_floors.items():
            print(f"  -> effective floor for {address}: {floor} GRAM (API floor: {api_floors.get(address)})")
